...
impl std::fmt::Display for Instruction {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        use Instruction::*;
        match self {
            Jump(a) =>              write!(f, "JMP 0x{:03X}", a),
            JumpPlus(a) =>          write!(f, "JMP V0 + 0x{:03X}", a),
            Call(a) =>              write!(f, "CAL 0x{:03X}", a),
            Return =>               write!(f, "RET"),
            
            SkipEqByte(x, b) =>     write!(f, "SEQ V{:X}, 0x{:02X}", x, b),
            SkipNeByte(x, b) =>     write!(f, "SNE V{:X}, 0x{:02X}", x, b),
            SkipEq(x, y) =>         write!(f, "SEQ V{:X}, V{:X}", x, y),
            SkipNe(x, y) =>         write!(f, "SNE V{:X}, V{:X}", x, y),
            SkipPressed(x) =>       write!(f, "SKP V{:X}", x),
            SkipNPressed(x) =>      write!(f, "SNP V{:X}", x),

            LoadByte(x, b) =>       write!(f, "MOV V{:X}, 0x{:02X}", x, b),
            LoadRandom(x, b) =>     write!(f, "RND V{:X}, 0x{:02X}", x, b),
            AddByte(x, b) =>        write!(f, "ADD V{:X}, 0x{:02X}", x, b),
            Copy(x, y) =>           write!(f, "MOV V{:X}, V{:X}", x, y),
            ...
            Subn(x, y) =>           write!(f, "SUN V{:X}, V{:X}", x, y),
            Shr(x) =>               write!(f, "SHR V{:X}", x),
            Shl(x) =>               write!(f, "SHL V{:X}", x),
            LoadDelay(x) =>         write!(f, "MOV V{:X}, DT", x),
            LoadPressed(x) =>       write!(f, "MOV V{:X}, K", x),
            SetDelay(x) =>          write!(f, "MOV DT, V{:X}", x),
            SetSound(x) =>          write!(f, "MOV ST, V{:X}", x),

            LoadI(a) =>             write!(f, "MOV I, 0x{:03X}", a),
            AddToI(x) =>            write!(f, "ADD I, V{:X}", x),
            SetSprite(x) =>         write!(f, "SPR V{:X}", x),
            StoreBCD(x) =>          write!(f, "BCD V{:X}", x),
            StoreRegisters(x) =>    write!(f, "STR {:X}", x),
            RestoreRegisters(x) =>  write!(f, "RST {:X}", x),
